...
namespace AccountsApi.Tests.V1.E2ETests
{
    ...
    public class DynamoDbAccountsIntegrationTests : DynamoDbIntegrationTests<Startup>
    {
        ...
        {
            return _fixture.Create<Account>();
        }
        ...
        {
            var accountDomain = ConstructAccount();
        ...
        {
            var accountDomain = new[] { ConstructAccount(), ConstructAccount() };
            ...
            {
                acc.TargetId = targetId;
                acc.AccountType = AccountType.Master;
                var guid = await CreateAccountAndValidateResponse(acc).ConfigureAwait(false);
                guids.Add(guid);
            }

            var uri = new Uri($"api/v1/accounts?targetId={targetId}&accountType=Master", UriKind.Relative);
